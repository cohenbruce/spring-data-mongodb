...
public class ReactiveMongoTemplate implements ReactiveMongoOperations, ApplicationContextAware {
	...
	private <T> Mono<T> doSaveVersioned(T objectToSave, MongoPersistentEntity<?> entity, String collectionName) {
		...
		return createMono(collectionName, collection -> {
			...
			Query query = new Query(
					Criteria.where(idProperty.getName()).is(id).and(versionProperty.getName()).is(version));
	...
	private void assertUpdateableIdIfNotSet(Object entity) {
		...
		Object idValue = persistentEntity.get().getPropertyAccessor(entity).getProperty(idProperty.get());

		if (idValue == null && !MongoSimpleTypes.AUTOGENERATED_ID_TYPES.contains(idProperty.get().getType())) {
	...
	private void initializeVersionProperty(Object entity) {
		...
		if (mongoPersistentEntity != null && mongoPersistentEntity.hasVersionProperty()) {
			...
			accessor.setProperty(mongoPersistentEntity.getVersionProperty().get(), 0);
		}
	}
